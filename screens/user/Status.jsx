...
export default function Status() {
	...
	useEffect(() => {
		...
		const subscription = supabase
			...
			.on(
				"postgres_changes",
				{ event: "*", schema: "public", table: "laundries_table" },
				(payload) => {
					getLaundry();
				}
			)
	...
	if (laundriesData?.length == 0) {
		return (
			<View style={{ alignItems: "center", marginTop: "30%" }}>
				...
				/>
				{/* </Surface>  */}
				<Text style={styles.text}>No laundry to check.</Text>
			</View>
		);
	}
		...
		<>
			<Provider>
				<ScrollView
					...
					refreshControl={
						<RefreshControl refreshing={reloading} onRefresh={getLaundry} />
					}>
					{/* TODO: DELETE */}
					<Portal>
						<StatusModal
					...
					{reloading ? (
						...
						laundriesData?.map((laundry) => {
							return (
								<TouchableOpacity
									activeOpacity={
										laundry.status == "pending" || laundry.status == "washing"
											? 1
											: 1
									}
									key={laundry.id}>
									<View
											...
											<View
												...
												<Text
													style={{
														...
														display:
															laundry.status == "pending" ? "none" : "flex",
													}}>
													{laundry.status == "done"
														? "Ready to pick up"
														: `${laundry.status}...`}
										...
										<Text
											style={[
												...
												{
													display:
														laundry.status == "pending" ? "flex" : "none",
												},
											]}
											onPress={() => {
												setToDelete(laundry);
